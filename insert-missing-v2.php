...
foreach ($missing_companies as $company) {
    ...
    foreach ($company_rows as $row_idx) {
        ...
        $stmt_insert = $conn->prepare("INSERT INTO delivery_records 
            (company_name, sold_to, invoice_no, item_code, quantity, serial_no, delivery_date, dataset_name)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        
        $dataset_name = '2024 to NOW BW Sales Record';
        $stmt_insert->bind_param('ssssisss', $company, $company, $invoice_no, $item, $qty, $serial_no, $delivery_date, $dataset_name);
    ...
    echo "  ✅ Inserted " . $company_inserted . " records\n";
}
...
// Get total records
$result_total = $conn->query("SELECT COUNT(*) as count FROM delivery_records WHERE dataset_name = '2024 to NOW BW Sales Record'");
$row_total = $result_total->fetch_assoc();
echo "Total records in dataset: " . $row_total['count'] . "\n";
?>
