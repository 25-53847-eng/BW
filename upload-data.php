...
// Get total record count from database (delivery_records + warranty_replacements)
require_once 'db_config.php';
$totalRecords = 0;

if ($conn) {
    // Count both tables - all records (not filtered by user for now to show full dataset)
    $result = @$conn->query("
        SELECT 
            (SELECT COUNT(*) FROM delivery_records) as delivery_count,
            (SELECT COUNT(*) FROM warranty_replacements) as warranty_count
    ");
    if ($result) {
        $row = $result->fetch_assoc();
        if ($row) {
            $delivery_count = intval($row['delivery_count'] ?? 0);
            $warranty_count = intval($row['warranty_count'] ?? 0);
            $totalRecords = $delivery_count + $warranty_count;
        }
    }
}
?>
        ...
        <div class="upload-container">
            ...
            <div class="upload-section preview-section" id="previewSection">
                ...
                <div class="dataset-name-input" style="margin-top: 20px; padding: 15px; background: rgba(244, 208, 63, 0.1); border: 1px solid rgba(244, 208, 63, 0.3); border-radius: 10px;">
                    ...
                    <input type="text" id="datasetNameInput" placeholder="Enter a name for this dataset (e.g., January Data, 2024 Sales)" 
                        style="width: 100%; padding: 12px 15px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.3); color: #fff; font-size: 14px; font-family: Verdana, sans-serif;">
            ...
            <div class="upload-section delete-section">
                ...
                <div class="delete-section-inner" style="display: flex; align-items: center; gap: 20px; padding: 20px; border-radius: 12px;">
                    ...
                    <button id="deleteAllDataBtn" type="button" onclick="showDeleteAllDataConfirm()" style="background: linear-gradient(135deg, #c62828 0%, #a61e1e 100%); color: #fff; padding: 12px 20px; border: none; border-radius: 8px; cursor: pointer; font-weight: 700; font-size: 14px; font-family: Verdana, sans-serif; transition: all 0.3s ease; display: flex; align-items: center; gap: 10px; white-space: nowrap; position: relative; z-index: 10001; box-shadow: 0 4px 16px rgba(166, 30, 30, 0.35); pointer-events: auto; touch-action: auto;">
                        <i class="fas fa-bomb"></i> Delete All Data
                    </button>
                    <button id="deleteAllBtn" type="button" onclick="showDeleteModal()" style="background: linear-gradient(135deg, #ff6b6b 0%, #ee5a24 100%); color: #fff; padding: 12px 25px; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; font-size: 14px; font-family: Verdana, sans-serif; transition: all 0.3s ease; display: flex; align-items: center; gap: 10px; white-space: nowrap; position: relative; z-index: 10001; pointer-events: auto; touch-action: auto;">
