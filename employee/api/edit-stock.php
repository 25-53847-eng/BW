...
// Check permission for editing inventory items
if (!isPermissionEnabled('inventory_edit_item', $conn)) {
    http_response_code(403);
    echo json_encode(['success' => false, 'error' => 'Permission denied. Admin has not granted you access to edit inventory items.']);
    exit;
}
...
try {
    ...
    if ($difference !== 0) {
        ...
        if ($recent && $row = $recent->fetch_assoc()) {
            ...
            $insert_result = $conn->query("INSERT INTO delivery_records (item_code, item_name, company_name, status, quantity, delivery_month, updated_at, notes, owner_user_id) VALUES ('{$item_code_esc}', '{$item_name_esc}', 'Stock Addition', 'Stored', {$current_stock}, '" . date('F') . "', CURRENT_TIMESTAMP, 'Manual Edit', {$owner_user_id})");
            if (!$insert_result) {
