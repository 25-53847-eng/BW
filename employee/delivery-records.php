...
// Employees should see ALL delivery records (not just their own)
$delivery_where = "company_name NOT IN ('Orders', 'Inquiry', 'Stock Addition') 
...
if ($itemResult) {
    while ($row = $itemResult->fetch_assoc()) {
        ...
        if (!in_array($code, $monthNames) && !empty($code) && !empty($name)) {
            ...
            $qty_query = "SELECT COALESCE(SUM(quantity), 0) as qty FROM delivery_records WHERE item_code = '" . $conn->real_escape_string($code) . "' AND company_name = 'Stock Addition'";
