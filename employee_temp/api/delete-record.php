...
// Check if record exists first and get company_name to determine permission
$check_stmt = $conn->prepare("SELECT company_name FROM delivery_records WHERE id = ?");
...
$requiredPermission = $isAndisonRecord ? 'andison_add_records' : 'delivery_delete_records';
if (!isPermissionEnabled($requiredPermission, $conn)) {
    http_response_code(403);
    $recordType = $isAndisonRecord ? 'Andison Manila records' : 'delivery records';
    echo json_encode(['success' => false, 'message' => "Permission denied. Admin has not granted you access to delete {$recordType}."]);
    exit();
}
