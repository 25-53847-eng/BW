...
// Check permissions for purchase order actions
$canViewPO = true;
$canEditPO = isPermissionEnabled('inventory_create_po', $conn);
$canDeletePO = isPermissionEnabled('inventory_create_po', $conn);
...
$autoEditMode = isset($_GET['edit']) && $canEditPO;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_po' && $canEditPO) {
        if (!$canEditPO) {
            $_SESSION['po_detail_flash'] = ['type' => 'error', 'message' => 'You do not have permission to edit purchase orders.'];
        ...
        if ($poStatus === 'Received') {
            ...
            if ($stmt) {
                $stmt->bind_param('ssisssi', $itemCode, $itemName, $quantity, $poNumber, $invoiceNo, $notes, $orderId);
                if (!$stmt->execute()) {
            ...
            // Note: Employee uses grouping logic similar to admin
            ...
            $check_stmt->bind_param("si", $itemCode, $owner_user_id);
            $check_stmt->execute();
            ...
            if ($item_exists) {
                ...
                $update_inv_sql = "UPDATE delivery_records SET quantity = quantity + ?, updated_at = CURRENT_TIMESTAMP WHERE item_code = ? AND company_name = 'Stock Addition' AND owner_user_id = ?";
                ...
                $upd_stmt->bind_param("isi", $quantity, $itemCode, $owner_user_id);
                $upd_stmt->execute();
                $upd_stmt->close();
            } else {
                // Create new Stock Addition record
                $company = 'Stock Addition';
                $insert_sql = "INSERT INTO delivery_records (delivery_month, delivery_day, delivery_year, item_code, item_name, quantity, company_name, status, groupings, uom, invoice_no, notes, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                ...
                $ins_stmt->bind_param("siissiisissss", $del_month, $del_day, $del_year, $itemCode, $itemName, $quantity, $company, $received_status, $grouping, $uom, $invoiceNo, $notes, $now, $now);
                if (!$ins_stmt->execute()) {
        ...
        $updateSql = "UPDATE delivery_records
                      SET item_code = ?, item_name = ?, quantity = ?, po_number = ?, invoice_no = ?,
                          po_status = ?, notes = ?, updated_at = CURRENT_TIMESTAMP
                      WHERE id = ? AND company_name IN ('Orders', 'Purchase Order')";

        $stmt = $conn->prepare($updateSql);
        if ($stmt) {
            $stmt->bind_param('ssisssi', $itemCode, $itemName, $quantity, $poNumber, $invoiceNo, $poStatus, $notes, $orderId);
            if ($stmt->execute()) {
    ...
    if ($action === 'delete_po' && $canDeletePO) {
        if (!$canDeletePO) {
            $_SESSION['po_detail_flash'] = ['type' => 'error', 'message' => 'You do not have permission to delete purchase orders.'];
            header('Location: po-details.php?id=' . $orderId, true, 302);
            exit;
        }
...
$fetch = $conn->prepare("SELECT * FROM delivery_records WHERE id = ? AND company_name IN ('Orders', 'Purchase Order') AND owner_user_id = ? LIMIT 1");
if (!$fetch) {
    die('Database error: ' . $conn->error);
}

$fetch->bind_param('ii', $orderId, $owner_user_id);
$fetch->execute();
        ...
        <main class="main-content" id="mainContent">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px;">
                <div>
                    ...
                    <p style="margin: 0; font-size: 13px; color: #7f8c8d; font-weight: 500;">View purchase order details</p>
                </div>
            ...
            <div style="max-width: 900px; margin: 0 auto;">
                ...
                <!-- EDIT MODE (Hidden by default, only shown if has permission) -->
                <?php if ($canViewPO): ?>
                <form id="editMode" method="post" style="<?php echo $autoEditMode ? 'display: block; ' : 'display: none; '; ?>background: linear-gradient(135deg, #f8f9fa 0%, #ffffff 100%); border-radius: 12px; border: 1px solid rgba(229, 231, 235, 0.8); padding: 32px; box-shadow: 0 2px 12px rgba(0, 0, 0, 0.06);">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 24px; margin-bottom: 28px;">
                        ...
                        <div>
                            ...
                            <select name="po_status" style="width: 100%; padding: 11px 14px; border: 1px solid #e5e7eb; border-radius: 8px; background: #ffffff; color: #2c3e50; font-size: 14px; font-family: inherit; transition: all 0.2s ease; box-sizing: border-box; cursor: pointer;" onfocus="this.style.borderColor='#5bbcff'; this.style.boxShadow='0 0 0 3px rgba(91, 188, 255, 0.1)';" onblur="this.style.borderColor='#e5e7eb'; this.style.boxShadow='none';">
                                ...
                                <option value="Pending" <?php echo ($order['po_status'] === 'Pending') ? 'selected' : ''; ?>>Pending</option>
                    ...
                    <div style="display: flex; gap: 12px; justify-content: center;">
                        ...
                        <?php endif; ?>
                        <a href="inventory.php?tab=orders" style="display: inline-flex; align-items: center; gap: 8px; padding: 12px 28px; border-radius: 8px; background: #e0e6ed; border: none; color: #2c3e50; font-weight: 600; font-size: 14px; cursor: pointer; transition: all 0.2s ease; text-decoration: none;" onmouseover="this.style.background='#d0d8e0'; this.style.transform='translateY(-2px)';" onmouseout="this.style.background='#e0e6ed'; this.style.transform='translateY(0)';">
